...
def get_classes(year: int, term: str, subjects: list[str]) -> pd.DataFrame:
    ...
    for elem in subject_course_elems:
        if "cf" in elem["class"]:
            ...
            description = (
                json_data.drop_duplicates("name")["description"].iat[0].strip()
            )
...
def read_major_classes(
    major_classes_file_path: str = DEFAULT_MAJOR_CLASSES_PATH,
) -> list[str]:
...
def read_taken_classes(
    taken_classes_file_path: str = DEFAULT_TAKEN_CLASSES_PATH,
) -> list[str]:
...
def select_major_classes(
    classes_df: pd.DataFrame, major_classes_file_path: str = DEFAULT_MAJOR_CLASSES_PATH
) -> pd.DataFrame:
...
def select_not_taken_classes(
    classes_df: pd.DataFrame, taken_classes_file_path: str = DEFAULT_TAKEN_CLASSES_PATH
) -> pd.DataFrame:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="output CSV file to write DataFrame to",
    )
    parser.add_argument(
        "-y", "--year", default=DEFAULT_YEAR, help="year to find classes for"
    )
    parser.add_argument(
        "-t",
        "--term",
        choices=["fall", "summer", "spring", "j-term"],
        default=DEFAULT_TERM,
        help="year to find classes for",
    )
    parser.add_argument(
        "-s",
        "--subjects",
        default=DEFAULT_SUBJECTS,
        help="subjects to find classes from",
    )
    parser.add_argument(
        "-q",
        "--quiet",
        action="store_true",
        help="suppress all printing. Still outputs to a CSV",
    )
    parser.add_argument(
        "-m",
        "--major",
    ...
    if args.major is not None:
        if not args.quiet:
            print(
                f"Showing only major classes. Reading major classes from {args.major}"
            )
